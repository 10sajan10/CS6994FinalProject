...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train per-site temporal transformer models on multihorizon discharge parquet data."
    )
    parser.add_argument(
        "--parquet-pattern",
    ...
    parser.add_argument(
        ...
        help="Project root where models_discharge/ and discharge_* metrics will be written.",
    )
...
def train_one_site(
    ...
    site_model_dir = output_root / "models_discharge" / str(site_id)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="discharge-lightning-best",
        monitor="val_loss",
        mode="min",
        save_top_k=1,
    )
    ...
    trainer = pl.Trainer(
        ...
        logger=CSVLogger(save_dir=str(output_root), name="lightning_logs_discharge"),
    ...
    best_model_path = site_model_dir / "discharge_best_model.pt"
...
def main() -> None:
    ...
    print("Starting discharge transformer training pipeline")
    ...
    metrics_path = output_root / "discharge_metrics_by_season.csv"
    ...
    horizon_metrics_path = output_root / "discharge_metrics_by_season_horizon.csv"
    horizon_metrics.to_csv(horizon_metrics_path, index=False)
    print(f"Saved horizon seasonal metrics to {horizon_metrics_path}")

    overall_metrics_path = output_root / "discharge_overall_validation_metrics.csv"
    ...
    expanded_predictions_path = output_root / "discharge_validation_predictions_long.csv"
